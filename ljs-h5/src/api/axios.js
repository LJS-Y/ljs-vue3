...
httpService.interceptors.response.use(
  (response) => {
    ...
    if (code === 200) {
      ...
      // Msg.warning('服务异常，请联系客服或稍后重试！');
      Msg.warning(LJSbase.fieldCheck(msg) ? '未知错误。' : msg);
  ...
  (error) => {
    ...
    if (error.msg === undefined) {
      Msg.error('服务异常，请稍候重试！');
    } else {
      Msg.error(error.msg + '，请稍候重试！');
    }
  }
);
...
export function download(url, params, filename, timeout = window.g.API_OUT_TIME, config) {
  ...
  return httpService({
    ...
    if (isLogin) {
      ...
      if (rspObj.code === 401) {
        Msg.info('您的访问权限已过期，请重新登录！');
        LJSsession.clearVuex(store);
        run.replace({
          path: '/login'
        });
      } else {
        Msg.error(rspObj.msg);
      }
    }
    downloadLoadingInstance.close();
  }).catch((r) => {
    Msg.error('下载文件出现错误，请联系管理员！')
    downloadLoadingInstance.close();
  });
}
