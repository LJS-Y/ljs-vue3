...
module.exports = {
	 ...
	 * */
	get(url, params = {}, timeout, isInternalRequest, responseInterceptor, headerParams) {
	 ...
	 * @param {string} timeout 超时时间，默认10秒
	 * */
	post(url, params = {}, submitDD, timeout, isInternalRequest, responseInterceptor) {
	 ...
	 * @param {string} timeout 超时时间，默认10秒
	 * */
	put(url, params = {}, submitDD, timeout, isInternalRequest, responseInterceptor) {
	 ...
	 * @param {string} submitDD 防抖动，是否需要开启
	 * */
	del(url, params = {}, submitDD, isInternalRequest, responseInterceptor) {
	 ...
	 * */
	uploadFile(url, name, filePath, timeout = 6000) {
		let header = {};
		// methodTag:LOGIN
		const token = store.getters.token; // token验证串
		if (true) {
	 ...
	 * @param {Number} timeout 超时时间。
	 * */
	uploadFiles(url, name, files, params = {}, timeout = 6000) {
