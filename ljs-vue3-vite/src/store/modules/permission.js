...
export function routerAndMenuChange(commit, res) {
  ...
  const routerData = dynamicRoutes.concat(interfaceData);
  commit('SET_routerList', routerData);
  routerData.forEach((r) => {
    router.addRoute('frame', r);
  });
  // 接口数据处理 - 菜单
  let useMenu = [];
  dynamicRoutes.forEach((m) => {
    if (m.hidden !== true) {
      useMenu.push(m);
    }
  });
...
export function loadView(modules, view) {
  const componentStr = `/src/views/${view}`;
  if (componentStr.indexOf('.vue') === -1) {
    return modules[`${componentStr}.vue`];
  } else {
    return modules[`${componentStr}`];
  }
}
