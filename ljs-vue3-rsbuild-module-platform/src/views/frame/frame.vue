    ...
    <div class="top">
      ...
      <div class="midd"
        ...
        <template v-for="(item, i) in moduleMenu.showData" :key="i">
          <div @click="$base.itemClick(item)" class="item" :class="{ active: $store.getters.module ? $store.getters.module.id === item.id : false }">
            <svg-icon class="icon" :iconName="item.icon !== ''?item.icon:'absent'"></svg-icon>
            <span class="name">{{item.moduleName}}</span>
          </div>
        </template>
        <el-dropdown popper-class="moduleMenuHidden" v-if="moduleMenu.showData.length < moduleMenu.data.length" trigger="click">
          <div class="item more">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item
                class="itemHidden"
                @click="$base.itemClick(item)"
                v-for="(item, i) in moduleMenu.hideData" :key="i">
                <svg-icon class="iconfont" :iconName="item.icon !== ''?item.icon:'absent'"></svg-icon>
                <span class="name">{{item.moduleName}}</span>
              </el-dropdown-item>
            </el-dropdown-menu>
          </template>
      ...
      <div class="right" ref="userInfoBox">
          ...
          <!-- <el-dropdown>
            ...
            <template #dropdown>
              <el-dropdown-menu>
                ...
                <el-dropdown-item @click="changeSkin('blue')" :style="{color: skin.value === 'blue' ? '#1890FF' : ''}">蓝色</el-dropdown-item>
        ...
        <div class="info">
          <img class="head" @click="goUserInfo" :src="userinfo.avatar" @error="$base.picError(userinfo, 'avatar', 'images/common/head.png')"/>
