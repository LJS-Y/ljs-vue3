...
export default {
  ...
  watch: {
    ...
    '$store.getters.menuList': {
			handler(n) {
        if (this.$store.getters.token !== null && this.$store.getters.menuList.length > 0) {
          this.init();
        }
			},
			deep: true
  ...
  methods: {
    ...
    changeSkin(theme) {
      ...
      this.$LJSsession.setLS('CCSW_DPT_FRAME_THEME', theme);
    },
    // 左侧菜单收缩
    menuShrink() {
    ...
    getModuleMenu() {
      this.$nextTick(() => {
        ...
        this.moduleMenu.data = this.$store.getters.modules;
        let moduleMenuW = 0;
        const userInfoBoxW = this.$refs.userInfoBox.offsetWidth; // 用户信息区域宽度
        const moduleLogoBoxW = this.$refs.moduleLogoBox.offsetWidth; // logo区域宽度
        this.moduleMenu.width = document.documentElement.clientWidth - userInfoBoxW - moduleLogoBoxW - 5; // 展示区域的宽度，5为间隙
        ...
        for (let i = 0; i < nums; i++) {
          ...
          // item宽度
          const moduleMenuItemW = moduleMenuIcoW + item.moduleName.length * 16 + moduleMenuPaddingW;
